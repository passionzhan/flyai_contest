...
from crf_accuracies import crf_marginal_accuracy
from crf_accuracies import crf_viterbi_accuracy
from test_utils import to_tuple
...
class CRF(Layer):
    ...
    def recursion(self, input_energy, mask=None, go_backwards=False,
        ...
        if mask is not None:
            mask2 = K.cast(K.concatenate([mask, K.zeros_like(mask[:, :1],dtype=mask.dtype)], axis=1),
                           K.floatx())
